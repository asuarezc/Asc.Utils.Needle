...
public interface INeedle
{
    ...
    ISemaphoreWorker GetSemaphoreWorker(int maxThreads, bool cancelPendingJobsIfAnyOtherFails);

    ISemaphoreWorker GetSemaphoreWorker(int maxThreads);

    ISemaphoreWorker GetSemaphoreWorker(bool cancelPendingJobsIfAnyOtherFails);

    ISemaphoreWorker GetSemaphoreWorker();

    IParallelWorker GetParallelWorker(bool cancelPendingJobsIfAnyOtherFails);

    IParallelWorker GetParallelWorker();

    /// <summary>
    /// Brings you access to the master needle worker singleton instance.
    /// </summary>
    IBackgroundSemaphoreWorker MainBackgroundWorker { get; }
}