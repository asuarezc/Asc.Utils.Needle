...
public interface INeedleWorker : INotifyPropertyChanged, IDisposable
{
    ...
    /// <param name="priority">Priority to run job</param>
    ...
    void AddJob(Action job, JobPriority priority = JobPriority.Medium);
    ...
    /// <exception cref="ArgumentNullException">If <paramref name="job"/> is null.</exception>
    /// <exception cref="InvalidOperationException">If worker is running.</exception>
    void AddJob(Func<Task> job, JobPriority priority = JobPriority.Medium);
